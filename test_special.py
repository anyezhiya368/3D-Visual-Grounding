...
def test(model, model_fn, data_name, epoch):
    false_pos = {}
    true_pos = {}
    for label_name in semantic_label_names:
        false_pos[label_name] = 0
        true_pos[label_name] = 0
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            for label in labels_unique:
                if label < 2:
                    continue
                semantic_name = semantic_label_names[label]
                label_mask = torch.nonzero(labels == label).view(-1)
                label_pred = semantic_pred[label_mask]
                num_label = label_pred.shape[0]
                correct_mask = torch.nonzero(label_pred == label).view(-1)
                label_correct = label_pred[correct_mask]
                num_background = label_correct.shape[0]
                false_pos[semantic_name] += num_background
                true_pos[semantic_name] += num_label
        ...
        # writer.add_scalar('map', all_ap_avg, cfg.test_epoch)
        # writer.add_scalar('map_50', all_ap_50o, cfg.test_epoch)
        # writer.add_scalar('map_25', all_ap_25o, cfg.test_epoch)

        for label in semantic_label_names:
            if true_pos[label] > 0:
                print(label, ':', false_pos[label] / true_pos[label])
